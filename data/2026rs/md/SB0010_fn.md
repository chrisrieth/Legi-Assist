...
This bill establishes a 120-day timeframe for the State Highway Administration (SHA) to 
approve or deny an application submitted by a program administrator or the program 
administrator’s contractor (or return an application as incomplete) when a county applies 
to obtain SHA’s approval to use a speed monitoring system on a State highway at a location 
within a municipal corporation. If SHA does not approve, deny, or return an application 
during that 120-day timeframe, the application is deemed approved. The bill also specifies 
that (1) a speed monitoring system contractor that submits an application must consult with 
the program administrator before and after the submission of an application and (2) a 
program administrator or contractor may not submit more than 10 applications during any 
30-day period. 
...
“Program administrator” means an employee or representative of the local jurisdiction, 
SHA, or Maryland Transportation Authority designated to oversee a contract with a speed 
